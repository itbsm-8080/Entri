...
function TfrmFp.getmaxkode:string;
var
  s:string;
begin
...
function TfrmFp.GetCDSSalesman: TClientDataset;
var s:String;
begin
...
begin
  ...
  S := 'select brg_kode sku, brg_nama NamaBarang, sum(mst_stok_in-mst_stok_out) Stok ,'
      + ' brg_hrgJual HrgJual,'
      + ' brg_satuan Satuan from tbarang left join tmasterstok on mst_brg_kode=brg_kode'
      + ' group by brg_kode '
      + ' order by brg_nama ';
...
procedure TfrmFp.clQTYPropertiesEditValueChanged(Sender: TObject);
var
  i: Integer;
  lVal: Double;
begin
...
procedure TfrmFp.HapusRecord1Click(Sender: TObject);
begin
...
procedure TfrmFp.rorte(Sender: TObject;
...
procedure TfrmFp.cxGrdMainDataControllerSummaryAfterSummary(
  ASender: TcxDataSummary);
...
procedure TfrmFp.edtDiscFakturExit(Sender: TObject);
begin
...
procedure TfrmFp.chkPajakClick(Sender: TObject);
begin
if not Floaddataall then
begin
//ubahharga;
...
if FLAGEDIT then
  s:='update TSO_HDR set '
    ...
    + ' so_freight = ' + FloatToStr(cStrToFloat(edtFreight.text)) + ','
...
begin
  ...
  s :=  ' insert into TSO_HDR '
             + ' (so_nomor,so_tanggal,so_jthtempo,so_memo,so_cus_kode,so_shipaddress,so_sls_kode,so_disc_faktur,'
             + ' so_disc_fakturpr,so_amount,so_freight,so_taxamount,so_DP,so_rek_dp,so_istax,so_isproforma,date_create,user_create) '
             + ' values ( '
             + Quot(edtNomor.Text) + ','
             + Quotd(dtTanggal.Date) + ','
             + Quotd(dtJthTempo.Date) + ','
             + Quot(edtmemo.Text)+','
...
begin
  ...
  s := ' select so_nomor,so_tanggal,so_memo,so_cus_kode,so_sls_kode,'
     ...
     + ' sod_brg_kode,sod_bRG_satuan,sod_qty,sod_harga,sod_discPR,(sod_qty*sod_harga*(100-sod_discpr)/100) nilai,so_freight'
     + ' from tso_hdr a'
   ...
   try
       ...
       begin
         ...
         begin
            ...
            edtfreight.text := fieldbyname('so_freight').AsString;
...
procedure TfrmFp.doslipSO(anomor : string );
var
  s: string ;
  ftsreport : TTSReport;
begin
...
function TfrmFp.GetCDSRekening: TClientDataset;
var s:String;
begin
...
procedure TfrmFp.doslipSO3(anomor : string );
var
  s: string ;
  tsql2,tsql : TMyQuery;
  abaris,i,a:Integer;
...
procedure TfrmFp.Button1Click(Sender: TObject);
  var
    s:string;
    tsql:TMyQuery;
    i:Integer;
...
procedure TfrmFp.cxButton4Click(Sender: TObject);
var
  s:string;
begin
  try
...
procedure TfrmFp.cxLookupCustomerExit(Sender: TObject);
begin
  if ceklocked(cxLookupCustomer.EditValue) then
  begin
    ShowMessage('Customer ini terkunci');
...
procedure TfrmFp.doslipSO2(anomor : string );
var
  s: string ;
  tsql2,tsql : TMyQuery;
  abaris,i,a:Integer;
...
function TfrmFp.ceklocked(akode:string):boolean;
var s:String;
tsql :TMyQuery;
begin
      Result := False;
...
procedure TfrmFp.doslipSO4(anomor : string );
var
  s: string ;
  tsql2,tsql : TMyQuery;
  abaris,i,a:Integer;
...
begin
 ...
 begin
  ...
  begin
    try
      memo.Lines.Add(StrPadRight('' , 38 , ' ')+ ' '+StrPadRight(' F A K T U R   P E N J U A L A N ', 40 , ' '));
  ...
  try
    ...
    begin
      TERBILANG := fieldbyname('terbilang').AsString;
          anilai := fieldbyname('FP_amount').AsFloat-fieldbyname('FP_taxamount').AsFloat-fieldbyname('FP_freight').AsFloat;
          appn := fieldbyname('FP_taxamount').AsFloat;
          atotal := fieldbyname('FP_amount').AsFloat-fieldbyname('FP_taxamount').AsFloat;
          afreight := fieldbyname('FP_freight').AsFloat;
          adp :=fieldbyname('FP_dp').AsFloat;
          arekening := fieldbyname('rekening').asstring;
          adiscfaktur :=  ((fieldbyname('FP_disc_fakturpr').asfloat*fieldbyname('nett').asfloat)/100) + fieldbyname('FP_disc_faktur').asfloat;

      memo.lines.add('Nomor     : ' + StrPadRight(fieldbyname('FP_nomor').AsString, 32, ' ')+StrPadRight('', 35, ' ')+ ' ' + StrPadRight('Customer : '+ fieldbyname('cus_nama').AsString, 45, ' '));
      memo.Lines.Add('Tanggal   : ' + StrPadRight(FormatDateTime('dd/mm/yyyy',fieldbyname('FP_tanggal').AsDateTime), 32, ' ')+StrPadRight('', 35, ' ')+ ' ' + StrPadRight(copy(fieldbyname('cus_alamat').AsString,1,40), 45, ' '));
      if fieldbyname('top').asinteger > 30 then
      memo.Lines.Add('Jth Tempo : ' + StrPadRight(FormatDateTime('dd/mm/yyyy',fieldbyname('FP_tanggal').AsDateTime+30), 32, ' ')+StrPadRight('', 36, ' ')+ StrPadRight(copy(fieldbyname('cus_alamat').AsString,41,40), 45, ' '))
      else
      memo.Lines.Add('Jth Tempo : ' + StrPadRight(FormatDateTime('dd/mm/yyyy',fieldbyname('FP_jthtempo').AsDateTime), 32, ' ')+StrPadRight('', 36, ' ')+ StrPadRight(copy(fieldbyname('cus_alamat').AsString,41,40), 45, ' '));
      memo.lines.add('Memo      : ' + StrPadRight(fieldbyname('fp_memo').AsString, 32, ' ')+ StrPadRight('', 36, ' ')+ StrPadRight('Salesman : '+ fieldbyname('sls_nama').AsString, 20, ' '));
      memo.Lines.Add(StrPadRight('', 120, '-'));
      memo.Lines.Add(StrPadRight('No', 3, ' ')+' '
                          +StrPadRight('Kode', 8, ' ')+' '
                          +StrPadRight('Nama', 44, ' ')+' '
...
//            memo.Lines.Add(StrPadcenter('S A L E S   O R D E R ', 120, ' '));
          memo.lines.add('Nomor : ' + StrPadRight(fieldbyname('FP_nomor').AsString, 32, ' ')+ ' ' + StrPadRight('Customer : '+ fieldbyname('cus_nama').AsString, 45, ' '));
          memo.Lines.Add('Tanggal : ' + StrPadRight(FormatDateTime('dd/mm/yyyy',fieldbyname('FP_tanggal').AsDateTime), 32, ' ')+ ' ' + StrPadRight(copy(fieldbyname('cus_alamat').AsString,1,40), 45, ' '));
          if fieldbyname('top').asinteger > 30 then
          memo.Lines.Add('Jth Tempo  : ' + StrPadRight(FormatDateTime('dd/mm/yyyy',fieldbyname('FP_tanggal').AsDateTime+30), 32, ' ')+ StrPadRight(copy(fieldbyname('cus_alamat').AsString,41,40), 45, ' '))
          else
          memo.Lines.Add('Jth Tempo  : ' + StrPadRight(FormatDateTime('dd/mm/yyyy',fieldbyname('FP_jthtempo').AsDateTime), 32, ' ')+ StrPadRight(copy(fieldbyname('cus_alamat').AsString,41,40), 45, ' '));
          memo.lines.add(StrPadRight('Memo : '+ fieldbyname('fp_memo').AsString, 36, ' ')+ StrPadRight('Salesman : '+ fieldbyname('sls_nama').AsString, 20, ' '));
...
//                          );
    ...
    memo.Lines.Add(
    StrPadRight('  Disiapkan', 20, ' ')
    +StrPadRight('   Menyetujui', 25, ' ')
    +StrPadRight('  Diantar', 23, ' ')
    +StrPadRight('Diterima', 25, ' '));
    memo.Lines.Add('');
    memo.Lines.Add('');
          memo.Lines.Add(
